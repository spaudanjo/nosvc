class PaymentDetail < ActiveRecord::Base
  ...
  def display_service_tax
    number_to_currency service_tax_amount, :unit => "EUR", :precision => 2, :delimiter => '.'
  end

  def display_net_amount
    number_to_currency net_amount, :unit => "EUR", :precision => 2, :delimiter => '.'
  end

  def display_total_amount
    number_to_currency total_amount, :unit => "EUR", :precision => 2, :delimiter => '.'
  end
